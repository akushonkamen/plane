...
- **输出**: 详细的实施步骤、代码示例、注意事项，但不会实际修改文件或运行命令
...
### 项目结构
Plane 是一个 pnpm monorepo (Turborepo)，包含以下结构：
...
pnpm turbo run dev --filter=@plane/ui  # 启动特定包的开发服务器

# 构建
pnpm build            # 构建所有包和应用
pnpm turbo run build --filter=@plane/ui  # 构建特定包
...
# Storybook
pnpm --filter=@plane/ui storybook  # 在端口 6006 启动 Storybook
...
- **状态管理**: MobX stores 在 `packages/shared-state` 中，使用响应式模式
- **测试**: 所有功能都需要单元测试，使用各包现有的测试框架
- **组件**: 在 `@plane/ui` 中构建，使用 Storybook 进行隔离开发
